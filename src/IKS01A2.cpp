    ...
    address(deviceAddress)
{
    ...
    writeRequest(address, LSM6DSLRegister::LSM6DSL_INT1_CTRL, std::vector<uint8_t>{0x02});
    // Accelerometer ODR=52 Hz, +-2g
    // Gyroscope ODR=52 Hz, full scale=250 dps
    writeRequest(address, LSM6DSLRegister::LSM6DSL_CTRL1_XL, std::vector<uint8_t>{0x30, 0x30});
    // Gyroscope HPF=65 mHz
    writeRequest(address, LSM6DSLRegister::LSM6DSL_CTRL7_G, std::vector<uint8_t>{0x50});
}
    ...
    address(deviceAddress)
{
    ...
    writeRequest(address, LSM303AGRRegister::LSM303AGR_CFG_REG_A_M, std::vector<uint8_t>{0x08});
    // asynchronous data always valid
    writeRequest(address, LSM303AGRRegister::LSM303AGR_CFG_REG_C_M, std::vector<uint8_t>{0x10});
}
